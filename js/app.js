...
(function() {
  ...
  function setState(msg) {
    ...
    console.log("setState: JSON.parse");
    try {
      payload = JSON.parse(msg.payloadString);
      if (payload.state.desired.state == "on") {
        console.log("setState: Found state: on");
        $('div[data-toggle^=toggle]').removeClass("off");
      }
      if (payload.state.desired.state == "off") {
        console.log("setState: Found state: off");
  ...
  function ClientController(client, logs) {
    ...
    this.topicName = '$aws/things/' + this.thingName + '/shadow/update/accepted';
    this.message = null;
    this.color = "white";
    this.state = "off";
    this.msgs = [];
  ...
  ClientController.prototype.publish = function() {
    this.client.publish(this.topicName, '{"state":{"desired":{"color": "' + this.color + '"}}}');
  };
  ...
  ClientController.prototype.changeColor = function(color) {
    this.color = color;
    this.client.publish('$aws/things/' + this.thingName + '/shadow/update', '{"state":{"desired":{"color": "' + this.color + '"}}}');
  };

  ClientController.prototype.toggleState = function() {
    var newState = (this.state == "on") ? "off" : "on";
    console.log("LED toggled: "+this.state+" => "+newState)
    this.state = newState;
    this.client.publish('$aws/things/' + this.thingName + '/shadow/update', '{"state":{"desired":{"state": "' + newState + '"}}}');
  };
  ...
  */
  ...
  var app = angular.module('awsiot.sample', []);
  app.controller('AppController', AppController);
  app.directive('toggleLoader', function() {
